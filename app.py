...
def predict():
    try:
        ...
        # Customize the response based on your prediction
        result = 'Congratulations! No Heart Disease was detected in your body' if prediction[0] == 0 else 'We are sorry to inform you, Heart Disease was detected in your body'

        return jsonify({'result': result})  # Return JSON response

    except ValueError as ve:
        return jsonify({'error': f'Invalid input. {str(ve)}'})
